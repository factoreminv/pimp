...
class MyNet(nn.Module):
    ...
    def randomjump(self):
        ...
        D1 = np.diag(np.abs(np.random.randn(32)/2)+0.3)
        D1 = D1 / np.linalg.det(D1) ** (1 /  32)
        # print(D1)
        # print(np.linalg.det(D1))
        D2 = np.diag(np.abs(np.random.randn(32)/2)+0.3)
...
if __name__ == '__main__':
    ...
    train(lr = 0.002, maxepochs = 20000, trainlength = 1)